...
async typeBeneficiaryName(v: string) {
  await this.util.inputText(
    this.page,
    "//mat-form-field[.//mat-label[contains(.,'Beneficiary Name')]]//input",
    v
  );
...
async selectBeneficiaryBank(bank: string) {
  await this.page
    .locator("//mat-label[contains(.,'Beneficiary Bank')]/ancestor::mat-form-field")
    .click();

  await this.page
    .locator(`//mat-option//span[normalize-space()='${bank}']`)
    .click();
}
...
async selectCurrency(v: string) {
  // open
  await this.page
    .locator("//mat-label[normalize-space()='Currency']/ancestor::mat-form-field")
    .click();
  ...
  // click option
  await panel.locator(`//mat-option//span[normalize-space()='${v}']`).click();
 ...
 async selectBeneficiaryCountry(v: string) {
  // open dropdown
  await this.page
    .locator("//mat-label[normalize-space()='Beneficiary Country']/ancestor::mat-form-field")
    .click();

  // pick option from mat panel
  const panel = this.page.locator(".mat-mdc-select-panel");
  await panel.waitFor({ state: "visible", timeout: 10000 });

  const opt = panel.locator(`//mat-option//span[normalize-space()='${v}']`).first();
  ...
  async swiftFlow(data: {
    ...
    beneficiaryBankClearingCode?: string;
    address3?: string;
  ...
  await this.util.inputText(
    this.page,
    "//mat-label[normalize-space()='Beneficiary Name']/ancestor::mat-form-field//input",
    data.beneficiaryName
  );
}

if (data.beneficiaryBankClearingCode) {
  await this.util.inputText(
    this.page,
    "//mat-label[normalize-space()='Beneficiary Bank Clearing Code']/ancestor::mat-form-field//input",
    data.beneficiaryBankClearingCode
  );
}
...
// fill intermediary BIC first (required before check becomes clickable)
await this.page
  .locator("//mat-label[normalize-space()='Intermediary Bank BIC']/ancestor::mat-form-field//input")
  .fill(data.intermediaryBic);

// click the Intermediary "Check Code" (2nd one on the page)
const btn = this.page.getByRole("button", { name: "Check Code" }).nth(1);
...
// Charge Option (safe)
const chargeField = this.page.locator(
  "//mat-label[normalize-space()='Charge Option']/ancestor::mat-form-field"
);
await chargeField.scrollIntoViewIfNeeded();
await chargeField.click();

const opt = this.page.locator(
  `//mat-option//span[contains(normalize-space(), '${data.chargeOption} -')]`
).first();
...
async quickPayFromMyAccounts(paymentName: string, amount: string | number) {
  // find the quick pay item by name
  ...
  // click Pay
  await card.getByRole("button", { name: /^Pay$/i }).first().click();
}
...
async confirmQuickPay() {
  const dlg = this.page
    ...
    .filter({ hasText: /Please Confirm|Confirm Transaction/i })
    .first();

  await dlg.waitFor({ state: "visible", timeout: 30000 });
  await dlg.getByRole("button", { name: /^Confirm$/i }).first().click();
}
