...
export class LoginPage {
  ...
  async goto(): Promise<void> {
    ...
    if (!onApp) {
      ...
      await this.page.goto('/BWInternet380/', { waitUntil: 'domcontentloaded', timeout: 100000 });
    }
  ...
  async login(username: string, password: string): Promise<void> {
    ...
    if (await this.isVisible(this.errorBanner, 4000)) {
      const popupText = await this.errorBanner.innerText();
      throw new Error("[LOGIN FAILED] "+popupText);
      //await allure.attachment('Login Popup Message', popupText, 'text/plain');
    }
