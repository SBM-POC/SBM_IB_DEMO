...
test.describe('Other Local Bank Transfer - Successful', () => {
    const scenarios = readCsvData('TestDataOtherLocalBankTransfer.csv');
        ...
        test(`Scenario ${index + 1}: ${scenarioLabel}`, async ({ page }, testInfo) => {
            ...
            try {
                ...
                await allure.step('Verify amount deduction from debit account', async () => {
                    ...
                    const [finalLedgerBal, finalAvailableBal] = await utilityLibraryPage.CaptureBalance(page, data.fromAccountHint, "CURRENT AND SAVING");
                ...
                await allure.step('Verify details in transaction history panel for debit account', async () => {
                    await activityCalenderHelper.VerifyInTransactionHistoryPanel('debit', actualPaymentDate, data.fromAccountHint, data.currency, data.amount, data.remarks);
                });
                await allure.step('Verify details in calendar activity history for debit account', async () => {
                    ...
                    await activityCalenderHelper.VerifyInCalendarActivity('debit', actualPaymentDate, data.fromAccountNickName, data.currency, data.amount, data.remarks);
...
test.describe('Other Local Bank Transfer - Unsuccessful', () => {
    const scenarios = readCsvData('TestDataOtherLocalBankTransfer_Unsuccessful.csv');
    for (const [index, row] of scenarios.entries()) {
