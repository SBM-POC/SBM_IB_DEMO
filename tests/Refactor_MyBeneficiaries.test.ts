    ...
    test(`ADD #${i + 1}: ${clean(r.PaymentName)}`, async ({ page }) => {
      ...
      await allure.step(`Fill Form (${bank})`, async () => {
        if (bank === "Other SBM Account Transfer") {
          ...
          await ben.typePaymentName(clean(r.PaymentName));
          await ben.selectFromAccount(clean(r.FromAccount));
          await ben.typeAmount(clean(r.Amount));                       // ✅ MISSING TODAY
          await ben.typeBeneficiaryName(clean(r.BeneficiaryName));     // ✅ MISSING TODAY
          await ben.selectBeneficiaryBank(clean(r.BeneficiaryBank));   // ✅ MISSING TODAY
...
if (r.IntermediaryBIC) {
  const interBic = page.locator(
    "//mat-label[normalize-space()='Intermediary Bank BIC']/ancestor::mat-form-field//input"
  );

  await interBic.scrollIntoViewIfNeeded();
  await interBic.fill(clean(r.IntermediaryBIC));
}
...
test.describe("Quick Pay (CSV)", () => {
  ...
  for (const [i, r] of payRows.entries()) {
    test(`QUICK PAY #${i + 1}: ${clean(r.PaymentName)}`, async ({ page }) => {
      ...
      // go back to My Accounts (Quick Pay widget is there)
const myAccountsTab =
