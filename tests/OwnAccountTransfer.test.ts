...
test.describe('Own Bank Transfer — Positive Scenarios (CSV)', () => {
    ...
    for (const [i, row] of rows.entries()) {
        ...
        test(label, async ({ page }) => {
            ...
            const [initialLedgerBal, initialAvailableBal] = await allure.step('Verify account', async () => {
                ...
                const [ledger, available] = await utilityLibraryPage.CaptureBalance(page, (csv(row, 'fromAccount')), "accountType");
            ...
            await allure.step('Verify amount deduction from debit via search by ref ID', async () => {
                ...
                const [finalLedgerBal, finalAvailableBal] = await utilityLibraryPage.CaptureBalance(page, (csv(row, 'fromAccount')), "CURRENT AND SAVING");
                const ledgerBalDiff = await finalLedgerBal - initialLedgerBal;
                const availableBalDiff = await finalAvailableBal - initialAvailableBal;
                await utilityLibraryPage.VerifyExpectedActual(ledgerBalDiff.toString(), (csv(row, 'amount')));
...
// ========================= NEGATIVE FLOWS =========================
test.describe('Own Bank Transfer — Negative Scenarios (CSV)', () => {
...
// ========================= UI CHECK =========================
test.describe('UI Check for Own Bank Transfer (Based on account types)', () => {
