...
test.describe('Other SBM Account Transfer', () => {
    ...
    test(`Scenario ${index + 1}: ${scenarioLabel}`, async ({ page }, testInfo) => {
      ...
      try {
        const [initialLedgerBal, initialAvailableBal] = await allure.step('Login and verify account', async () => {
          ...
          const [ledger, available] = await utilityLibraryPage.CaptureBalance(page, data.senderAccount, "CURRENT AND SAVING");
        ...
        switch (true) {
          ...
          case flowType === 'happy':
            ...
            await allure.step('Verify amount deduction from debit via search by ref ID', async () => {
              ...
              const [finalLedgerBal, finalAvailableBal] = await utilityLibraryPage.CaptureBalance(page, data.senderAccount, "CURRENT AND SAVING");
            ...
            await allure.step('Verify details in transaction history panel for debit account', async () => {
              const actualPaymentDate = await utilityLibraryPage.CalculateDateDDMMYYYY(data.paymentDate);
              await activityCalenderHelper.VerifyInTransactionHistoryPanel('debit', actualPaymentDate, data.senderAccount, data.currency, data.amount, data.remarks);
            });
            await allure.step('Verify details in calendar activity history for debit account', async () => {
              ...
              await activityCalenderHelper.VerifyInCalendarActivity('debit', actualPaymentDate, data.senderNickname, data.currency, data.amount, data.remarks);
