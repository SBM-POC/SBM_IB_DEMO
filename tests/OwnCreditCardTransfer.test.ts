...
import { ActivityCalenderHelper } from '../Helpers/ActivityCalender';

let actualRefID: string;
let exchangeRatesArray : number;

test.describe('Pay Own Credit Card', () => {
  ...
  for (const [index, row] of scenarios.entries()) {
    ...
    test(`Scenario ${index + 1}: ${label}`, async ({ page }, testInfo) => {
      ...
      try {
        const [initialLedgerBal, initialAvailableBal] = await allure.step('Login and verify account', async () => {
          await loginPage.goto();
          await loginPage.login(row.Username, row.Password);
          await myAccount.assertMyAccountTabActive();
          const [ledger, available] = await utilityLibraryPage.CaptureBalance(page, data.fromAccount, "CURRENT AND SAVING");
        ...
        await allure.step('Fill Credit Card payment form and Proceed for Confirmation', async () => {
          ...
          await page.locator("button#fundstransfer_next").waitFor({ state: 'visible'});
          await page.locator("button#fundstransfer_next").click();

        });
        if (flowType === 'happy') {
                  
          const calculatedAmount = await utilityLibraryPage.CalculateAmountExchangeRate(data.exchangeRateType, exchangeRatesArray, data.amount);
          ...
          await allure.step('Verify amount deduction from debit via search by ref ID', async () => {
            await utilityLibraryPage.SelectTab(page, 'My Accounts');
            const [finalLedgerBal, finalAvailableBal] = await utilityLibraryPage.CaptureBalance(page, data.fromAccount, "CURRENT AND SAVING");
            const ledgerBalDiff = await utilityLibraryPage.DeductAmount(initialLedgerBal, finalLedgerBal);
            ...
            if (data.exchangeRateType.trim().length > 0) {
              ...
              await utilityLibraryPage.VerifyExpectedActual((ledgerBalDiff.toFixed(2)).toString(), data.amount);
              await utilityLibraryPage.VerifyExpectedActual((availableBalDiff.toFixed(2)).toString(), data.amount);
            }
          });
          await allure.step('Verify details in transaction history panel for debit account', async () => {
            await utilityLibraryPage.SelectTab(page, 'My Accounts');
            if (data.exchangeRateType.trim().length > 0) {
              await activityCalenderHelper.VerifyInTransactionHistoryPanel('debit', actualPaymentDate, data.fromAccount, data.fromAccountCurrency, calculatedAmount.toString(), data.remarks);
            }
            else {
              await activityCalenderHelper.VerifyInTransactionHistoryPanel('debit', actualPaymentDate, data.fromAccount, data.fromAccountCurrency, data.amount, data.remarks);
            }
          });
          await allure.step('Verify details in calendar activity history for debit account', async () => {
            ...
            if (data.exchangeRateType.trim().length > 0) {
              await activityCalenderHelper.VerifyInCalendarActivity('debit', actualPaymentDate, data.fromAccountNickname, data.fromAccountCurrency, calculatedAmount.toString(), data.remarks);
            }
            else {
              await activityCalenderHelper.VerifyInCalendarActivity('debit', actualPaymentDate, data.fromAccountNickname, data.fromAccountCurrency, data.amount, data.remarks);
            }
