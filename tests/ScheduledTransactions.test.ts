            ...
            try {
                ...
                if (isTrue) {
                    ...
                    await allure.step('Verify details in calendar activity history for debit account', async () => {
                        await utilityLibraryPage.SelectTab(page, 'My Accounts');
                        await waitForSpinnerToDisappear(page);
                        await activityCalenderHelper.VerifyInCalendarActivity('debit', paymentDate, senderNickname, currency,amount,remarks);        
                    });
                }
                else {
...
test.describe('Other Local Bank Transfer - Scheduled Transactions', () => {
    let actualRefID: string;
    const scenarios = readCsvData('TestDataOtherLocalBankTransfer_Scheduled.csv');
    for (const [index, row] of scenarios.entries()) {
        const scenarioLabel = row.Description?.replace(/\s+/g, ' ').trim();
        test(`Scenario ${index + 1}: ${scenarioLabel}`, async ({ page }, testInfo) => {
            const loginPage = new LoginPage(page);
            ...
            try {
                const [initialLedgerBal, initialAvailableBal] = await allure.step('Login and verify account', async () => {
                    ...
                    await loginPage.login(row.Username, row.Password);
                    await myAccount.assertMyAccountTabActive();
                    const [ledger, available] = await utilityLibraryPage.CaptureBalance(page, data.fromAccountHint, "CURRENT AND SAVING");
                ...
                await allure.step('Validate successful dialog details', async () => {
                    ...
                    updateCsvCell('TestDataOtherLocalBankTransfer_Scheduled.csv',(index+1),"eBankingRefID",actualRefID) 
                ...
                await allure.step('Download receipt and verify receipt content', async () => {
                    ...
                    const currentTransactionDate = await utilityLibraryPage.CalculateDateMonthDDYYYY("TODAY");                    
                    const calculatedFutureDate= await utilityLibraryPage.CalculateDateDDMMYYYY(data.transactionDate)
                    const futurePaymentDate=await utilityLibraryPage.moveToNextWorkingDay_DDMonthYYYY(calculatedFutureDate)                                        
                    
                    await olb.verifyReceiptData(maskedFromAccount, maskedToAccount, currentTransactionDate, data.amount, data.currency, data.remarks, actualRefID, futurePaymentDate);
                    await pay.dialog.close();
                 ...
                 await allure.step('Verify transaction details in View Scheduled Transactions under Pay & Transfer', async () => {
                    await utilityLibraryPage.SelectTab(page, 'Pay & Transfer');
                    await utilityLibraryPage.SelectSubMenu(page, 'Scheduled Transactions');
                    await utilityLibraryPage.selectDropdown(page, `//mat-label[contains(text(),'From Account')]/../../mat-select`, data.fromAccountHint);
                    const optionToSelect = await page.locator(`mat-option:has-text("${data.fromAccountHint}")`);
                    await optionToSelect.scrollIntoViewIfNeeded();
                    await optionToSelect.click();
                    await waitForSpinnerToDisappear(page);
                    const calculatedFutureDate= await utilityLibraryPage.CalculateDateDDMMYYYY(data.transactionDate)
                    const futurePaymentDate=await utilityLibraryPage.moveToNextWorkingDay_DDMMYYYY(calculatedFutureDate)
                    await scheduledTransactionsPage.VerifyScheduledTransactionDetails(page,"Other Local Bank Transfer",futurePaymentDate,"Debit",data.currency,data.amount,data.remarks,data.recipientAccount);               
                });
                await allure.step('Verify transaction details in Scheduled Transactions in Accounts View', async () => {
                    await utilityLibraryPage.SelectTab(page, 'My Accounts');
                    await waitForSpinnerToDisappear(page);
...
test.describe('Other Local Bank Transfer - Verify Scheduled Transactions on Scheduled Date', () => {
    const scenarios = readCsvData('TestDataOtherLocalBankTransfer_Scheduled.csv');
    for (const [index, row] of scenarios.entries()) {
        const scenarioLabel = row.Description?.replace(/\s+/g, ' ').trim();
        test(`Scenario ${index + 1}: ${scenarioLabel}`, async ({ page }, testInfo) => {
...
test.describe('Own Bank Account Transfer — Schedule Transactions', () => {
    ...
    for (const [index, row] of scenarios.entries()) {
        ...
        test(`Scenario ${index + 1}: ${scenarioLabel}`, async ({ page }, testInfo) => {
            ...
            const [initialLedgerBal, initialAvailableBal] = await allure.step('Verify account', async () => {
                await myAccount.assertMyAccountTabActive();
                const [ledger, available] = await utilityLibraryPage.CaptureBalance(page, senderAccount, "accountType");
                return [ledger, available];
            ...
            try {
                ...
                if (isTrue) {
                    ...
                    await allure.step('Verify details in calendar activity history for debit account', async () => {
                        await utilityLibraryPage.SelectTab(page, 'My Accounts');
                        await waitForSpinnerToDisappear(page);
                        await activityCalenderHelper.VerifyInCalendarActivity('debit', paymentDate, senderAccount, currency,amount,remarks);        
                    });
                }
                else {
