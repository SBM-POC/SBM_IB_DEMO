...
const scenarios = readCsvData('Bill_Payment_testdata.csv');
...
test.describe('Bill Payments (CSV)', () => {
  for (const [i, row] of scenarios.entries()) {
    test(`Scenario ${i + 1}: ${row.Description ?? 'Bill Payment'}`, async ({ page }) => {
      ...
      // 2.5) TemplateQuickPay: pay a specific template by name
...
// 2.7) TemplateEdit: edit a template by name using provided columns
